...
def train(args, config):
    ...
    for epoch in range(max(0, last_epoch), 5_000_000):
        ...
        for i, batch in enumerate(train_loader):
            ...
            wav = get_segments(
                x=wav.unsqueeze(1),
                start_idxs=output["z_start_idxs"] * (
                    generator.module.upsample_factor if hasattr(generator, "module") else generator.upsample_factor),
                segment_size=output["segment_size"] * (
                    generator.module.upsample_factor if hasattr(generator, "module") else generator.upsample_factor),
            )
            ...
            y_df_hat_r, y_df_hat_g, fmap_f_r, fmap_f_g, y_ds_hat_r, y_ds_hat_g, fmap_s_r, fmap_s_g = discriminator(wav,
                                                                                                                   output[
                                                                                                                       "wav_predictions"].detach())
            ...
            if rank == 0:
                ...
                if iteration % (config.iters_per_checkpoint) == 0:
                    ...
                    save_checkpoint(checkpoint_path, {
                        'generator': (generator.module if hasattr(generator, 'module') else generator).state_dict()})
