...
def main():
    args = argparse.Namespace(data_dir='.', exp_dir='../exp')
...
def prepare_info(data_dir, info_dir):
    ...
    for name in ["emotion", "energy", "pitch", "speed", "tokenlist"]:
        shutil.copy(f"{ROOT_DIR}/youdao/text/{name}", f"{info_dir}/{name}")
    ...
    with open(f"{ROOT_DIR}/youdao/text/speaker2") as f, \
...
def prepare_config(data_dir, info_dir, exp_dir, config_dir):
    ...
    with open(f"{ROOT_DIR}/template_config.py") as f, \
            open(f"{config_dir}/config.py", "w") as fout:
        for line in f:
            fout.write(
                line.replace('<DATA_DIR>', data_dir).replace('<INFO_DIR>', info_dir).replace('<EXP_DIR>', exp_dir))
